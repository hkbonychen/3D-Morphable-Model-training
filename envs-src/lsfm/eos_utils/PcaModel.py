...
def rescale_pca_basis(orthonormal_basis, eigenvalues):
    """
    Takes an orthonormal PCA basis matrix (a matrix consisting of the eigenvectors) and rescales it, i.e. multiplies each
    eigenvector by the square root of its corresponding eigenvalue.
    Args:
...
def normalise_pca_basis(rescaled_basis, eigenvalues):
    """
    Takes a rescaled PCA basis matrix and scales it back to an orthonormal basis matrix by multiplying each eigenvector
...
class PcaModel:
    ...
    Attributes:
        mean: A 3m x 1 col-vector (xyzxyz...)', where m is the number of model-vertices.
        orthonormal_pca_basis: (eigenvectors). m x n (rows x cols) = numShapeDims x numShapePcaCoeffs. Each column is an eigenvector.
        eigenvalues: A col-vector of the eigenvalues (variances in the PCA space).
        triangle_list: List of triangles that make up the mesh of the model.
        rescaled_pca_basis: m x n (rows x cols) = numShapeDims x numShapePcaCoeffs. Each column is an eigenvector.
    '"""
    ...
    def get_rescaled_pca_basis_at_point(self, vertex_id):
        ...
        Note: The function does not return the i-th basis vector, it returns all basis vectors, but only the block that is relevant for the vertex \p vertex_id.
        Args:
    ...
    def draw_sample(self, coefficients: np.ndarray) -> np.ndarray:
        ...
        The given coefficients should follow a standard normal distrib, i.e. not be "normalised" with their eigenvalues/variances.
        Args:
            coefficients: The PCA coefficients used to generate the sample, ndarray type.
        Returns:
